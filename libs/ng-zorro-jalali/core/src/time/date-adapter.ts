...
export type DateLocaleFns = Locale;
...
export abstract class NzDateAdapter<D = Date> {
  ...
  abstract parse(
    text: string,
    formatStr: string,
    options?: { locale: DateLocaleFns; weekStartsOn: number }
  ): D;
  ...
  abstract setQuarter(date: D, amount: number): D;
...
export class DateFnsDateAdapter extends NzDateAdapter<Date> {
  ...
  parse(
    text: string,
    formatStr: string,
    options: { locale: DateLocaleFns; weekStartsOn: WeekDayIndex }
  ): Date {
