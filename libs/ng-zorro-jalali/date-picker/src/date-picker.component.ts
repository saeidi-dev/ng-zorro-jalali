...
export class NzDatePickerComponent implements OnInit, OnChanges, AfterViewInit, ControlValueAccessor {
  ...
  ngOnInit(): void {
    ...
    this.datePickerService.emitValue$.pipe(takeUntilDestroyed(this.destroyRef)).subscribe(() => {
      ...
      if (
        !this.isRange &&
        (value as CandyDate)?.isSame((datePickerPreviousValue as CandyDate), granularityComparison)
      ) {
        this.onTouchedFn();
        return this.close();
      }
      ...
      if (this.isRange) {
        ...
        if (
          previousStartDate?.isSame(currentStartDate, granularityComparison) &&
          previousEndDate?.isSame(currentEndDate, granularityComparison)
        ) {
          this.onTouchedFn();
          return this.close();
        }
  ...
  setModeAndFormat(): void {
    const inputFormats: Partial<Record<NzDateMode, string>> = {
      ...
      quarter: this.dateFormats.quarterLabel,
      week: this.dateFormats.weekYearLabel, // Format for week
