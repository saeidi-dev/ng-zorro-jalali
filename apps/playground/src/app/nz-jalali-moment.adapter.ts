...
import { DateMode, NzDateAdapter } from 'ng-zorro-jalali/core';
...
export class JalaliMomentDateAdapter extends NzDateAdapter<Moment> {
  ...
  getQuarter(date: moment.Moment): number {
    return date.get('quarter');
  }
  ...
  setQuarter(date: Moment, quarter: number): Moment {
    return date.clone().quarter(quarter);
  }
